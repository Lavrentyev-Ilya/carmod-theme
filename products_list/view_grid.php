...
<div class="cm_gridView">
    ...
    <?foreach($aRes['PRODUCTS'] as $PKEY=>$aProd){?>
        <div class="partCard CmAdmButsProduct" <?=isAdmButs_Product_x($aProd)?> >
			...
			<div class="CmImgLogoProd_c">
			   <div class="CmBrandLogoLeftBl" style="background-image:url(<?=$aProd['Logo']?>)">
					<div class="CmInfoBlockGrid" data-artnum="<?=$aProd['ArtNum']?>" data-brand="<?=$aProd['Brand']?>" data-link="<?=$aProd['Link']?>">
						...
						<div class="analogButt cm_evenButt CmTitShow" title="<?=Lng_x('Lookup_analogues');?>">
							<a class="CmLookAnalogHook" href="<?=$aProd['Link']?>#ProductsList">
								<svg class="material-icon analog_x" viewBox="0 -2 24 24">
                ...
                <?if($aProd['Schema_src'] || $aProd['Image']){?>
                    <div class="img_bl img_blHov cm_curMove ProductImg BoxGlr_n">
                        <?if($aProd['Schema_src']&&$aProd['Schema_src']!=''){?>
                            <div class="CmSchemaCoordsWrap">
                                <img alt="x" class="CmProdIm GlrImg_n" src="<?=$aProd['Schema_src']?>" alt="<?=$aProd['Brand'].' '.$aProd['ArtNum'].' - '.$aProd['Name'].' '.$_SERVER['SERVER_NAME']?>" data-pictype="schema" style="">
                                <div class="CmCoordBlock"><?=$aProd['Schema_html']?></div>
                            </div>
                        <?}else{?>
                            <img alt="x" class="CmProdIm GlrImg_n" src="<?=$aProd['Image']?>" alt="<?=$aProd['Brand'].' '.$aProd['ArtNum'].' - '.$aProd['Name'].' '.$_SERVER['SERVER_NAME']?>">
                        <?}?>
            ...
            <?if($aProd['Brand']||$aProd['ArtNum']||$aProd['Name']){ ?>
                <div class="CmGridLink">
                    <a href="<?=$aProd['Link']?>" class="CmGridBrandArt">
                        <div><?=$aProd['Brand']?> <?=$aProd['ArtNum']?></div>
                        <h2 class="CmGridProdName"><?=$aProd['Name']?></h2>
                    </a>
                </div>
            <?}else{?><div class="CmEmptyGridBlock"></div><?}?>
            <div class="CmListPrTab_c" data-price-view="card" data-price-context="products_list" <?if($aProd['WsAjax']){?>data-wsajax="1" data-dir="<?=CM_DIR?>" data-artnum="<?=$aProd['ArtNum']?>" data-brand="<?=$aProd['Brand']?>"<?}?> data-act="<?=$aRes['ACTIVE_TAB']?>">
                <?$PodPriceNum++;
				$ProductURL = GetProductLink_x($aProd); //AddCart URL
                PrintPrices_x(array('CONTEXT'=>'products_list','VIEW'=>'card','PRODUCT'=>$aProd,'RES'=>$aRes,'PRODUCT_URL'=>$ProductURL,'AJAX_CUT'=>false));?>
            </div>
        </div>
    <?}?>
    </div>
